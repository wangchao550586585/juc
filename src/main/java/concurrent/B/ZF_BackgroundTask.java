 ...
 * 可以在后台运行长时间任务,不会影响gui响应性
 * @author Brian Goetz and Tim Peierls
 */

public abstract class ZF_BackgroundTask <V> implements Runnable, Future<V> {
        ...
        public Computation() {
            super(new Callable<V>() {
                public V call() throws Exception {
                    return ZF_BackgroundTask.this.compute();
                }
            });
        }

        protected final void done() {
            ZF_GuiExecutor.instance().execute(new Runnable() {
                public void run() {
                    V value = null;
